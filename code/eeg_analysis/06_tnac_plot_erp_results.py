...
# --- GLOBAL SETTINGS
# --- SET PATH to data
data_path = '/media/cmusci/ceccmusci/ga_trial/'


# ========================================================================
# execute this cell to load previously saved epochs data
#with open(data_path + 'mne_tnac_results/erp_data_dict.pkl', 'rb') as erp:
#with open(data_path + 'mne_tnac_results_musicians/erp_musicians_data_dict.pkl', 'rb') as erp: #musicians group
    #erp_data_dict = pickle.load(erp)
    #erp_musicians_data_dict = pickle.load(erp) #musicians group

with open(data_path + '/mne_tnac_results_rightnonmus/erp_rightnonmus_data_dict.pkl', 'rb') as erp:  # musicians group
    # erp_data_dict = pickle.load(erp)
    erp_rightnonmus_data_dict = pickle.load(erp)  # musicians group

# extract epochs data - all partic
# music = erp_data_dict['music'][0]
# song = erp_data_dict['song'][0]
# voice = erp_data_dict['voice'][0]

#group : musicians
#music = erp_musicians_data_dict['music'][0]
#song = erp_musicians_data_dict['song'][0]
#voice = erp_musicians_data_dict['voice'][0]

#group : non musicians
#music = erp_non_mus_data_dict['music'][0]
#song = erp_non_mus_data_dict['song'][0]
#voice = erp_non_mus_data_dict['voice'][0]

#music = erp_left_data_dict['music'][0]
#song = erp_left_data_dict['song'][0]
#voice = erp_left_data_dict['voice'][0]


music = erp_rightnonmus_data_dict['music'][0]
song = erp_rightnonmus_data_dict['song'][0]
voice = erp_rightnonmus_data_dict['voice'][0]

# clean up
#del erp_data_dict, erp
#del erp_musicians_data_dict, erp # for musicians group
#del erp_non_mus_data_dict, erp # for musicians group

del erp_rightnonmus_data_dict, erp
# .copy().apply_baseline(baseline=(-0.3, -0.05))
# .copy().apply_baseline(baseline=(-0.3, -0.05))

#--- stelle die nicht klappt bei Musicians
#--------------------------------------------

# --- compute individual erps music ---
individual_erps_music = [music[i].average() for i in range(0, len(music))]
# grand average ERP
ERP_music = mne.grand_average(individual_erps_music)

# --- compute individual erps voice ---
individual_erps_voice = [voice[i].average() for i in range(0, len(voice))]
# grand average ERP cue B
ERP_voice = mne.grand_average(individual_erps_voice)

# --- compute individual erps song---
individual_erps_song = [song[i].average() for i in range(0, len(song))]
# grand average ERP cue B
ERP_song = mne.grand_average(individual_erps_song)
...
topomap_args = dict(sensors=False,
                    ...
                    average=0.05)
# times to plot
ttp = [.300, .350, .400, .450, .500, .600, .700]

# save figure cue A
fig_a = ERP_music.plot_joint(times=ttp,
                             ts_args=ts_args,
                             topomap_args=topomap_args,
                             title='Right-handers Non Musicians: Average Evoked Activity - Music Stimuli')
fig_a.axes[0].set_xticks(list(np.arange(-.25, 2.05, .25)), minor=False)
fig_a.axes[0].set_yticks(list(np.arange(-5, 5.5, 2.5)), minor=False)
fig_a.axes[0].axhline(y=0, xmin=-.5, xmax=2., color='black', linestyle='dotted')
...
#fig_a.savefig(data_path + 'plots/music_erp.pdf', dpi=300)
#fig_a.savefig(data_path + 'plots/music_erp_musicians.pdf', dpi=300)
fig_a.savefig(data_path + 'plots/music_erp_rightnonmus.pdf', dpi=300)

# save figure cue B
fig_b = ERP_voice.plot_joint(times=ttp,
                             ts_args=ts_args,
                             topomap_args=topomap_args,
                             title='Right-handers Non Musicians: Average Evoked Activity - Voice Stimuli')
fig_b.axes[0].set_xticks(list(np.arange(-.25, 2.05, .25)), minor=False)
fig_b.axes[0].set_yticks(list(np.arange(-5, 5.5, 2.5)), minor=False)
...
#fig_b.savefig(data_path + 'plots/voice_erp.pdf', dpi=300)
#fig_b.savefig(data_path + 'plots/voice_erp_musicians.pdf', dpi=300)
fig_b.savefig(data_path + 'plots/voice_erp_rightnonmus.pdf', dpi=300)

# save figure cue B
fig_c = ERP_song.plot_joint(times=ttp,
                            ts_args=ts_args,
                            topomap_args=topomap_args,
                            title='Right-handers Non Musicians: Average Evoked Activity - Song Stimuli')
fig_c.axes[0].set_xticks(list(np.arange(-.25, 2.05, .25)), minor=False)
fig_c.axes[0].set_yticks(list(np.arange(-5, 5.5, 2.5)), minor=False)
fig_c.axes[0].axhline(y=0, xmin=-.5, xmax=2.5, color='black', linestyle='dotted')
fig_c.axes[0].axvline(x=0, ymin=-5, ymax=5, color='black', linestyle='dotted')
# fig_b.axes[0].annotate('GFP', xy=(-.25, -8))
sns.despine(offset=5, trim=True)
fig_c.set_size_inches(9, 4.5)
#fig_c.savefig(data_path + 'plots/song_erp.pdf', dpi=300)
#fig_c.savefig(data_path + 'plots/song_erp_musicians.pdf', dpi=300)
fig_c.savefig(data_path + 'plots/song_erp_rightnonmus.pdf', dpi=300)
